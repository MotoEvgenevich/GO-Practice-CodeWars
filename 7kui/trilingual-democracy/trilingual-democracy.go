...
func TrilingualDemocracy(group string) string {
	// Создаем map для подсчета количества каждого языка
	counts := make(map[rune]int)
	for _, lang := range group {
		counts[lang]++
	}

	// Применяем правила на основе подсчетов
	switch len(counts) {
	case 1:
		// Все три человека говорят на одном языке
		for lang := range counts {
			return string(lang)
		}
	case 2:
		// Два человека говорят на одном языке, третий — на другом
		for lang, count := range counts {
			if count == 1 {
				return string(lang) // Возвращаем язык, на котором говорит только один человек
			}
		}
	case 3:
		// Три человека говорят на разных языках, выбираем четвертый
		languages := "DFIK"
		for _, lang := range languages {
			if _, exists := counts[lang]; !exists {
				return string(lang) // Возвращаем язык, который никто не знает
			}
		}
	}
	return "?" // Непредвиденный случай, для безопасности
}
