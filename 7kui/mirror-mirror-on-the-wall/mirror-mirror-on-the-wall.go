...
func Mirror(data []int) []int {
	...
	result := append([]int{}, sortedData...)

	for i := len(sortedData) - 2; i >= 0; i-- {
...
You get a list of integers, and you have to write a function mirror that returns the "mirror" (or symmetric) version of this list: i.e.
the middle element is the greatest, then the next greatest on both sides, then the next greatest, and so on...
