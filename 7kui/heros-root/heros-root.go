...
func IntRac(n uint64, guess int) int {
	// Преобразуем n в int64 для целочисленного деления
	var x int = guess
	var count int = 0

	for {
		// Вычисляем новое целое приближение
		newX := (x + int(n)/x) / 2

		// Увеличиваем счетчик шагов
		count++

		// Если разница между старым и новым значением меньше 1, то выходим из цикла
		if int(math.Abs(float64(x-newX))) < 1 {
			break
		}
...
(x + n / x) / 2; we are finished when the previous x and the new x have an absolute difference
less than e.

We supply to a function (int_rac) a number n (positive integer) and a parameter guess
(positive integer) which will be our initial x. For this kata the parameter 'e' is set to 1.

Hero's algorithm is not always going to come to an exactly correct result! For instance:
if n = 25 we get 5 but for n = 26 we also get 5. Nevertheless 5 is the integer square root of 26.
...
same results as in the tests and the other languages.
*/
